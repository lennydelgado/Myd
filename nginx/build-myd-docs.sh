...
# Recovery of requirements file
export REQUIREMENTS_FILE=$(echo "$GIT_REPO" | cut -d'/' -f1-5)
export BRANCH_NAME=$(echo "$GIT_REPO" | cut -d'/' -f7 | cut -d'.' -f1)
REQUIREMENTS_FILE=$REQUIREMENTS_FILE"/raw/"$BRANCH_NAME"/requirements.txt"
...
RES=$?
if ! [ "$RES" -eq "0" ]; then
    sh nginx/./first-build-myd-mkdocs.sh ${PYTHON_VERSION} ${REPO_DOCKER_URL} ${GIT_TOKEN} ${REQUIREMENTS_FILE}
fi;
...
if [ "$RES" -eq "0" ]; then
    docker image build --build-arg CACHEBUST=$(date +%s) --build-arg REPO_DOCKER_URL --build-arg PYTHON_VERSION --build-arg GIT_TOKEN --build-arg REQUIREMENTS_FILE -f $PWD/nginx/myd-mkdocs.dockerfile -t myd-mkdocs:latest . 2> $PWD/logs/nginx_build_log.txt
...
docker image build --build-arg CACHEBUST=$(date +%s) --build-arg COMMIT_MESSAGE --build-arg PROJ_NAME --build-arg GIT_PAGE_REPO --build-arg GIT_MAIL --build-arg GIT_USERNAME --build-arg REPO_DOCKER_URL --build-arg GIT_TOKEN --build-arg GIT_REPO --build-arg GIT_ZIP_NAME --build-arg DIR_NAME -f $PWD/nginx/myd-docs.dockerfile -t myd-docs:latest .
